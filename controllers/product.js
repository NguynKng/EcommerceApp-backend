...
const getProduct = async (req, res) => {
    try {
        ...
        const products = await productModel.find(filter).sort({name:1}).populate("images")
    ...
    try {
        ...
        const data = await page.evaluate(() => {
            return Array.from(document.querySelectorAll(".item > a")).map(item => ({
                ...
                img: item.querySelector(".item-img > img")?.getAttribute("src") || "No Image",
        ...
        console.error(err);
        return res.status(500).json({ success: false, message: "Error crawling data." });
    }
};
...
const getFeaturedCollections = async (req, res) => {
    try {
        ...
        const populatedProducts = await productModel.populate(products, { path: "images" });

        return res.status(200).json({ success: true, products: populatedProducts });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ success: false, message: "Error fetching collections." });
    }
};
...
module.exports = { addProduct, getProductByID, getCrawlData, getProduct, getProductBySlug, updateProductById, deleteProductById, getFeaturedCollections, getSpecialProduct, getPopularProduct };